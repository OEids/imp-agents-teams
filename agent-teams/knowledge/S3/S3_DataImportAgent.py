"""
S3 Data Import Agent
Recognizes non-template format grants and calculations data
Handles: Grants, Funding Allocations, Budget Scenarios, Planning Data
"""
...
class S3DataImportAgent:
    """
    Agent for importing non-template format data into Strand 3
    Specializes in grants, funding, and planning data
    """
    ...
    def __init__(self, strand_folder: str = None):
        ...
        # CSV structure mappings for S3 (Grants & Planning)
        self.csv_schemas = {
            'Grants': ['GrantCode', 'Title', 'GrantType', 'FundingYear', 'Amount', 'GrantEnabled'],
            'FundingAllocations': ['AllocationCode', 'SchoolCode', 'GrantCode', 'Amount', 
                                   'AllocationDate', 'AllocationEnabled'],
            'BudgetScenarios': ['ScenarioCode', 'Title', 'Year', 'Quarter', 'ScenarioEnabled'],
            'PlanningData': ['PlanningCode', 'Category', 'SubCategory', 'Value', 'PlanningYear', 'PlanningEnabled'],
            'Savings': ['SavingCode', 'Title', 'Amount', 'ImplementationDate', 'SavingEnabled'],
            'Priorities': ['PriorityCode', 'Title', 'Category', 'AllocatedBudget', 'PriorityEnabled'],
            'PhaseData': ['PhaseCode', 'Title', 'PhaseSequence', 'Completed', 'PhaseEnabled'],
            'Calculations': ['CalculationCode', 'Title', 'Formula', 'CalculationEnabled'],
            'AdjustmentData': ['AdjustmentCode', 'Category', 'AdjustmentAmount', 'Reason', 'AdjustmentEnabled']
        }
        
        self.strand_name = "Strand 3"
    
    def _locate_template(self) -> str:
        """Locate the Strand 3 template Excel file"""
        for file in os.listdir(self.strand_folder):
            if 'Strand 3' in file and 'Standard Workbook' in file and file.endswith('.xlsx'):
                return os.path.join(self.strand_folder, file)
            # Also check for generic COR004 naming
            if 'COR004' in file and 'Strand 3' in file and file.endswith('.xlsx'):
                return os.path.join(self.strand_folder, file)
        raise FileNotFoundError("Strand 3 template workbook not found")
            ...
            try:
                with open(guide_path, 'r') as f:
                    ...
                    if 'CSV IS the goal' in self.mission_guide:
                        print("✓ S3 Agent: Mission understood - CSV standardization is the goal")
            except:
    ...
    def detect_data_format(self, data: pd.DataFrame) -> Tuple[str, bool]:
        """
        Detect if data is grants/planning-related and format
        ...
        if any('grant' in col for col in lower_cols):
            return ('Grants', False)
        elif any('allocation' in col or 'funding' in col for col in lower_cols):
            return ('FundingAllocations', False)
        elif any('scenario' in col or 'budget' in col for col in lower_cols):
            return ('BudgetScenarios', False)
        elif any('saving' in col for col in lower_cols):
            return ('Savings', False)
        elif any('priority' in col for col in lower_cols):
            return ('Priorities', False)
        elif any('phase' in col for col in lower_cols):
            return ('PhaseData', False)
        elif any('calculation' in col or 'formula' in col for col in lower_cols):
            return ('Calculations', False)
        elif any('adjustment' in col for col in lower_cols):
            return ('AdjustmentData', False)
        elif any('plan' in col for col in lower_cols):
            return ('PlanningData', False)
        ...
        for target_col in target_columns:
            if target_col in customer_data.columns:
                ...
                for cust_col in customer_data.columns:
                    ...
                    elif 'Amount' in target_col or 'Budget' in target_col:
                        mapped_data[target_col] = 0.0
                    else:
    ...
    def _columns_match(self, target: str, source: str) -> bool:
        ...
        target_clean = target.lower().replace('code', '').replace('title', '').replace('enabled', '').replace('data', '')
        source_clean = source.lower().replace('code', '').replace('title', '').replace('enabled', '').replace('data', '')
    ...
    def process_customer_data(self, customer_data: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            if not import_file:
                # Create new import file if it doesn't exist
                import_file = data_type
            ...
            # Step 4: Update/create import file
...
def import_customer_data(file_path: str, strand_folder: str = None) -> Dict:
    ...
    Args:
        ...
        strand_folder: Path to strand folder (defaults to S3)
    
    Returns:
        Status report
    """
    agent = S3DataImportAgent(strand_folder)
...
if __name__ == "__main__":
    # Example usage
    print("S3 Data Import Agent initialized")
